...
export function createCursorController(opts: {
  ...
  const pickSmallestRawNodeIdAtClientPoint = (model0: any, x: number, y: number): string | null => {
    ...
    const compId = opts.getCompositeEditId();
    ...
    for (let i = 0; i < els.length; i++) {
      ...
      if (!rawId) continue;
      // Composite/group edit: the composite ROOT should behave like background (its interactions are hidden).
      // Only its internal sub-elements and real child nodes should be interactive.
      if (compId && rawId === compId) continue;
      if (gid) {
      ...
      if (type === "arrow" || type === "line") {
        ...
        // clicking anywhere on the arrow node counts as a hit (even if not within the 20px segment band).
  ...
  const updateFromClientPoint = (clientX: number, clientY: number) => {
    ...
    if (opts.getCompositeEditId()) {
      ...
      // Composite edit: hovering the composite ROOT `.node` itself should not show an action cursor.
      // (The root is a reference frame; only internal subs/children are meant to be edited.)
      const compId = opts.getCompositeEditId();
      const hitNode = elAt.closest<HTMLElement>(".node");
      if (compId && hitNode && String(hitNode.dataset.nodeId ?? "") === String(compId)) {
        opts.stage.style.cursor = "";
        return;
      }
    }
