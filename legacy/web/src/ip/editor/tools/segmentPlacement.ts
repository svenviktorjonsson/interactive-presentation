...
export function createSegmentPlacementController(opts: {
  ...
  let segmentDraft:
    ...
    | {
        ...
        kind: "arrow" | "line";
        nSegments: number;
        // Arrow drag-out / finalize support:
        // - drag-out: first pointerdown + drag + release creates the arrow
        // - click-place: first click starts draft, second click (or click-drag-release) finalizes
      ...
      if (!segmentDraft) {
        ...
        if (segmentDraft.kind !== kind || segmentDraft.space !== space) {
          ...
          if (kind === "line") {
            ...
            // - always ignore (no-op)
            if (isOnStart) {
              // Keep drafting (no-op) to avoid emitting zero-length segment.
            } else if (isOnOrigin && segmentDraft.nSegments > 0) {
              // Close the loop by snapping back to origin, but KEEP drafting.
              // Only Esc / right-click cancels the draft.
              // IMPORTANT: while in draw mode we must not re-introduce selection UI (no selection box).
              opts.addLineFromTo(toCommitPoint(start, space, scr), toCommitPoint(segmentDraft.origin, space, scr), { space, select: false });
              segmentDraft.start = segmentDraft.origin;
              segmentDraft.nSegments += 1;
              updateSegmentPreview(segmentDraft, originScreenNow);
            } else {
              ...
              segmentDraft.currentEnd = pos;
              segmentDraft.nSegments += 1;
              updateSegmentPreview(segmentDraft, posScreen);
            }
          } else {
            // Arrow behavior:
            // - second click starts finalization (arrow materializes on pointerup)
            segmentDraft.currentEnd = pos;
            segmentDraft.dragPointerId = ev.pointerId;
            segmentDraft.dragStartClient = { x: ev.clientX, y: ev.clientY };
            segmentDraft.dragMoved = false;
            segmentDraft.finalizeOnPointerUp = true;
            updateSegmentPreview(segmentDraft, posScreen);
          }
        }
      }
  ...
  const onPointerMove = (ev: PointerEvent) => {
    if (opts.getAppMode() !== "edit") return;
    const tool = opts.getTool();
    const canShow = tool === "line" || tool === "arrow" || !!segmentDraft;
    if (!canShow) return;
    ...
    // Hover ring is only for line drafting.
    const showHoverRing = tool === "line" || (segmentDraft && segmentDraft.kind === "line");
    if (showHoverRing) ensureHoverRing();
    let pos = toModelPoint(screenPos, space, cam as any, scr as any);
    const modelNow = engine.getModel();
    const tolPx = 12;
    const tolPx2 = tolPx * tolPx;
    ...
    if (segmentDraft) {
      ...
      updateSegmentPreview(segmentDraft, posScreen);
      segmentDraft.currentEnd = pos as any;
      // Arrow drag-out / drag-finalize detection (only while the left button is held).
      if (segmentDraft.kind === "arrow" && (ev.buttons & 1) && segmentDraft.dragPointerId === ev.pointerId && segmentDraft.dragStartClient) {
        const dx = ev.clientX - segmentDraft.dragStartClient.x;
        const dy = ev.clientY - segmentDraft.dragStartClient.y;
        if (Math.hypot(dx, dy) >= 3.0) segmentDraft.dragMoved = true;
      }
  ...
  const onPointerUp = (ev: PointerEvent) => {
    ...
    if (ev.button !== 0) return;
    if (segmentDraft.kind !== "arrow") return;
    if (segmentDraft.dragPointerId !== ev.pointerId) return;

    const cam = engine.getCamera();
    const scr = engine.getScreen();
    const space = segmentDraft.space;
    const start = segmentDraft.start;
    const end = segmentDraft.currentEnd;

    const shouldFinalize = segmentDraft.finalizeOnPointerUp || segmentDraft.dragMoved;
    if (!shouldFinalize) {
      // First click without drag: keep drafting (preview follows mouse until second click).
      segmentDraft.dragPointerId = null;
      segmentDraft.dragStartClient = null;
      segmentDraft.dragMoved = false;
      segmentDraft.finalizeOnPointerUp = false;
      void cam;
      return;
    }
    ...
    segmentDraft = null;
    opts.addArrowFromTo(toCommitPoint(start, space, scr), toCommitPoint(end, space, scr), { space });
    ev.preventDefault();
    (ev as any).stopImmediatePropagation?.();
  };
  ...
  const onContextMenu = (ev: MouseEvent) => {
    ...
    // No active draft -> let the state machine handle tool exit / context menus.
  };
  ...
  return {
    ...
    onPointerUp: (ev) => (onPointerUp(ev), !!(ev as any).defaultPrevented),
