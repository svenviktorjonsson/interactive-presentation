...
def _repo_root() -> Path:
    # legacy/backend/app/content_loader.py -> repo root
...
def _ensure_choices_composite_defaults(
    ...
    *,
    ...
    elements_path = comp_dir / "elements.pr"
...
def _ensure_wheel_composite_defaults(
    ...
    *,
    ...
    - wheel/elements.pr: text templates for each option id + "other"
    """
    if not poll_id:
        return
    base = pres_dir / "groups" / str(poll_id) / "wheel"
    ...
    # --- geometries.csv (label offsets) ---
    geo_path = base / "geometries.csv"
    try:
        ...
        if geo_path.exists():
            ...
            existing_rows = [
                {"id": "pie", "view": "composite", "x": "0.5", "y": "0.5", "w": "1", "h": "1", "rotationDeg": "0", "anchor": "centerCenter", "align": "center", "parent": ""}
            ]
            have_ids.add("pie")

        def add_label_row(label_id: str) -> None:
            if label_id in have_ids:
                return
            have_ids.add(label_id)
            existing_rows.append(
                {
                    "id": label_id,
                    "view": "composite",
                    # Offsets from computed anchor (0,0 default).
                    "x": "0",
                    "y": "0",
                    # Default label box size (relative to wheel group box).
                    "w": "0.36",
                    "h": "0.10",
                    "rotationDeg": "0",
                    "anchor": "centerCenter",
        ...
        with geo_path.open("w", encoding="utf-8", newline="") as f:
            w = csv.DictWriter(f, fieldnames=fieldnames)
            w.writeheader()
            for row in existing_rows:
                out = {k: row.get(k, "") for k in fieldnames}
                out["view"] = out.get("view") or "composite"
                w.writerow(out)
    except Exception:
        pass
...
def _parse_presentation_txt(path: Path, *, design_w: float, design_h: float) -> dict[str, Any]:
    ...
    while i < len(lines):
        ...
        if kw == "table":
            ...
            # Syntax:
            # table[name=...,delim=;,hstyle=||c|c||,vstyle=b||c|...||]:
            # A;B;C
            # 1;2;3
            content_lines: list[str] = []
            inline = (m.group("inline") or "").strip()
            ...
            delim = str(params.get("delim") or params.get("delimiter") or ";")
            hstyle = str(params.get("hstyle") or params.get("hStyle") or "").strip()
            vstyle = str(params.get("vstyle") or params.get("vStyle") or "").strip()
            rows: list[list[str]] = []
            for ln in content_lines:
                ...
                if not s and rows:
                    continue
                rows.append([c.strip() for c in s.split(delim)])
            ...
            nodes_by_id[name] = {
                ...
                "rows": rows,
                "delimiter": delim,
                "hstyle": hstyle or None,
                "vstyle": vstyle or None,
            }
            _apply_style_params(nodes_by_id[name], params)
            if current_view:
                current_view["show"].append(name)
        ...
        if kw == "choices":
            ...
            try:
                ...
                try:
                    ...
                    if elp.exists():
                        nodes_by_id[name]["wheelElementsPr"] = elp.read_text(encoding="utf-8")
                except Exception:
                    pass
            except Exception:
                pass
