...
export function createDomNode(node: NodeModel): DomNodeHandle | null {
  ...
  if (node.type === "graph") {
    // Graph reuses the timer "base widget" visuals (frame + data region overlay + canvas)
    // so it looks identical to the timer composite, minus the control buttons and timer-specific stats.
    const el = document.createElement("div");
    el.classList.add("node-graph");
    el.style.boxSizing = "border-box";

    const frame = document.createElement("div");
    // Reuse timer frame styling so graph matches timer 1:1.
    ...
    // Reuse timer canvas sizing/styling.
  ...
  if (node.type === "sound") {
    const el = document.createElement("div");
    el.classList.add("node-sound");
    el.style.boxSizing = "border-box";

    const frame = document.createElement("div");
    frame.className = "sound-frame";
    frame.style.position = "absolute";
    frame.style.inset = "0";
    frame.style.overflow = "visible";
    frame.style.borderRadius = "0";
    ...
    const overlayBg = document.createElement("div");
    overlayBg.className = "sound-overlay-bg";
    overlayBg.style.position = "absolute";
    overlayBg.style.left = "8%";
    overlayBg.style.right = "8%";
    overlayBg.style.top = "10%";
...
function _anchorToTopLeftFrac(t: { x: number; y: number; w: number; h: number; anchor?: string }) {
  const a =
    (t.anchor ?? "topLeft") === "top"
      ? "topCenter"
      : (t.anchor ?? "topLeft") === "bottom"
        ? "bottomCenter"
        : t.anchor ?? "topLeft";
  switch (a) {
    case "center":
...
function _topLeftToAnchorFrac(rect: { x: number; y: number; w: number; h: number }, anchor?: string) {
  const a =
    (anchor ?? "topLeft") === "top"
      ? "topCenter"
...
export function layoutDomNodes(args: {
  ...
  for (const node of model.nodes) {
    ...
    if (nodeLayout.space === "world") {
      ...
      // Screen-space transforms are normalized fractions (0..1) relative to the current screen size.
      // Origin is top-left.
      const sx0 = Number(nodeLayout.transform.x ?? 0);
      const sy0 = Number(nodeLayout.transform.y ?? 0);
      const sw = Number(nodeLayout.transform.w ?? 0.2);
      const sh = Number(nodeLayout.transform.h ?? 0.1);
      const anchor = nodeLayout.transform.anchor ?? "topLeft";

      // Clamp screen nodes so at least 50% stays on-screen (so off-screen nodes remain editable).
      const tl0 = _anchorToTopLeftFrac({ x: sx0, y: sy0, w: sw, h: sh, anchor });
      const tlx = Math.max(-0.5 * sw, Math.min(1 - 0.5 * sw, tl0.x));
      const tly = Math.max(-0.5 * sh, Math.min(1 - 0.5 * sh, tl0.y));
      const ap = _topLeftToAnchorFrac({ x: tlx, y: tly, w: sw, h: sh }, anchor);
      const sx = ap.x;
      const sy = ap.y;

      const wPx = sw * screen.w;
      const hPx = sh * screen.h;
      const { dx, dy } = anchorOffsetPx(anchor, wPx, hPx);
      px = { x: sx * screen.w + dx, y: sy * screen.h + dy, w: wPx, h: hPx };
    }
    ...
    // Expose sound pixel size and scale for header layout.
    if (nodeLayout.type === "sound") {
      const baseH = Math.max(1e-6, Number(nodeLayout.transform.h ?? 1));
      const uiScale = px.h / baseH;
      handle.el.style.setProperty("--sound-scale", String(uiScale));
      handle.el.style.setProperty("--ui-scale", String(uiScale));
      handle.el.dataset.soundWpx = String(px.w);
      handle.el.dataset.soundHpx = String(px.h);
    }
    // Expose graph pixel size for overlay layout (axis labels/arrows are rendered by the app layer).
    if (nodeLayout.type === "graph") {
      handle.el.dataset.graphWpx = String(px.w);
      handle.el.dataset.graphHpx = String(px.h);
      const baseH = Math.max(1e-6, Number(nodeLayout.transform.h ?? 1));
      const uiScale = px.h / baseH;
      handle.el.style.setProperty("--ui-scale", String(uiScale));
    }
