...
export function niceTicks(minV: number, maxV: number, targetTicks: number, candidates: number[], fmt: (v: number) => string): Tick[] {
  const a = Number(minV);
  const b = Number(maxV);
...
export function drawGrid(ctx: CanvasRenderingContext2D, plot: PlotRectPx, dpr: number, xTicks: Array<{ xFrac: number }>, yTicks: Array<{ yFrac: number }>) {
...
export function drawTicksAndLabels(args: {
  ...
  const { ox, oy, xLen, yLen } = plot;
  const { fontPx } = timerLikeTickFontPx(rectCss, dpr);

  ctx.save();
  ctx.strokeStyle = "rgba(255,255,255,0.55)";
  ctx.fillStyle = "rgba(255,255,255,0.82)";
  ctx.lineWidth = Math.max(1, lineWidthPx * dpr);
  ...
  for (const t of xTicks) {
    ...
    ctx.stroke();
    ctx.fillText(String(t.label ?? ""), x, oy + tickLen + 2 * dpr);
  }
  ...
  for (const t of yTicks) {
    ...
    ctx.stroke();
    ctx.fillText(String(t.label ?? ""), ox - tickLen - 4 * dpr, y);
  }
