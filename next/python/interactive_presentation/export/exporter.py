...
def export_bundle(presentation_pr: str, out_dir: str = "dist") -> None:
  ...
  - Copies `next/web/dist` to out_dir
  - Writes `model.json`
  ...
  # exporter.py lives at: next/python/interactive_presentation/export/exporter.py
  # -> parents[3] is next/
  ...
  if not web_dist.exists():
    raise FileNotFoundError(f"web dist not found: {web_dist} (run `cd next/web && npm run build`)")
